...
# 目標値
df["target"] = iris.target.astype(str)  # 列全体を文字列に変換
...
# 予測モデル構築
x = pd.DataFrame(
    iris.data[:, [0, 2]], columns=["sepal length (cm)", "petal length (cm)"]
)
y = iris.target
...
value_df = pd.DataFrame(
    [[sepalValue, petalValue]], columns=["sepal length (cm)", "petal length (cm)"]
)
